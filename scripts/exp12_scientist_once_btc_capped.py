...
from deep_orderbook.scientist_experiment import apply_prediction_cap, choose_latest_parquet, richness_gate
...
async def main() -> None:
    ...
    precheck_path = Path(f"experiments/pictures/exp12_scientist_once_btc_capped_precheck_{stamp}.png")
    ...
    notes_path = notes_dir / f"exp12_scientist_once_btc_capped_{stamp}.md"
    result_path = Path(f"experiments/results/exp12_scientist_once_btc_capped_{stamp}.json")
    dashboard_path = Path(f"experiments/pictures/exp12_scientist_once_btc_capped_dashboard_{stamp}.png")

    if not richness["usable"]:
        notes = f"# exp12 scientist once btc capped — rejected before training\n\n- timestamp: {timestamp_utc}\n- parquet: `{selected_parquet}`\n- hypothesis: After BTC showed interesting raw structure but catastrophic RMSE, capping prediction amplitude to a train-derived percentile may preserve structure while controlling dense-map blow-up.\n- observation: richness gate failed with reason `{richness['reason']}`\n- precheck image: `{precheck_path}`\n- decision: reject slice before training\n"
        notes_path.write_text(notes)
        payload = {
            "experiment": "exp12_scientist_once_btc_capped",
    ...
    notes = "# exp12 scientist once btc capped\n\n" + "\n".join([
        f"- timestamp: {timestamp_utc}",
        f"- parquet: `{selected_parquet}`",
        f"- symbol: `BTC-USD`",
        f"- mutation: prediction amplitude cap at train q99.5",
    ...
    row_id = register_experiment_run(
        experiment="exp12_scientist_once_btc_capped",
        variant="tinytcn_h64_once_btc_capped",
        timestamp_utc=timestamp_utc,
        git_commit=git_commit,
        symbol="BTC-USD",
        cadence="100ms",
