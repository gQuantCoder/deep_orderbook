...
async def main() -> None:
    ...
    precheck_path = Path(f"experiments/pictures/exp13_multifile_btc_walkforward_precheck_{stamp}.png")
    ...
    notes_path = notes_dir / f"exp13_multifile_btc_walkforward_{stamp}.md"
    result_path = Path(f"experiments/results/exp13_multifile_btc_walkforward_{stamp}.json")
    dashboard_best_path = Path(f"experiments/pictures/exp13_multifile_btc_walkforward_best_{stamp}.png")
    dashboard_fixed_path = Path(f"experiments/pictures/exp13_multifile_btc_walkforward_fixed_{stamp}.png")
    ...
    if not richness["usable"]:
        ...
        payload = {
            "experiment": "exp13_multifile_btc_walkforward",
    ...
    notes = "# exp13 multifile btc walkforward\n\n" + "\n".join([
        f"- timestamp: {timestamp_utc}",
        f"- train files:",
        *[f"  - `{name}`" for name in train_file_names],
        f"- test file: `{test_file.name}`",
        f"- symbol: `BTC-USD`",
        f"- mutation: walk-forward multi-file holdout + prediction cap at train q99.5",
    ...
    payload = {
        ...
        "richness_gate": richness,
        "mutation": {
            "type": "walkforward_multifile_holdout_plus_prediction_cap",
    ...
    row_id = register_experiment_run(
        experiment="exp13_multifile_btc_walkforward",
        variant="tinytcn_h64_btc_walkforward_capped",
        timestamp_utc=timestamp_utc,
        git_commit=git_commit,
        symbol="BTC-USD",
        cadence="100ms",
