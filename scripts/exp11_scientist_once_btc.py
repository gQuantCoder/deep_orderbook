...
async def main() -> None:
    ...
    precheck_path = Path(f"experiments/pictures/exp11_scientist_once_btc_precheck_{stamp}.png")
    ...
    notes_path = notes_dir / f"exp11_scientist_once_btc_{stamp}.md"
    result_path = Path(f"experiments/results/exp11_scientist_once_btc_{stamp}.json")
    dashboard_path = Path(f"experiments/pictures/exp11_scientist_once_btc_dashboard_{stamp}.png")

    if not richness["usable"]:
        notes = f"# exp11 scientist once btc — rejected before training\n\n- timestamp: {timestamp_utc}\n- parquet: `{selected_parquet}`\n- hypothesis: After the weak ETH run, switching only the symbol to BTC-USD on the same fresh parquet may reveal richer tradable structure.\n- observation: richness gate failed with reason `{richness['reason']}`\n- precheck image: `{precheck_path}`\n- decision: reject slice before training\n"
        notes_path.write_text(notes)
        payload = {
            "experiment": "exp11_scientist_once_btc",
    ...
    notes = "# exp11 scientist once btc\n\n" + "\n".join([
