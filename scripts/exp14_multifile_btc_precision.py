...
async def main():
    ...
    precheck_path = Path(f"experiments/pictures/exp14_multifile_btc_precision_precheck_{stamp}.png")
    ...
    notes_path = notes_dir / f"exp14_multifile_btc_precision_{stamp}.md"
    result_path = Path(f"experiments/results/exp14_multifile_btc_precision_{stamp}.json")
    dashboard_best_path = Path(f"experiments/pictures/exp14_multifile_btc_precision_best_{stamp}.png")
    dashboard_fixed_path = Path(f"experiments/pictures/exp14_multifile_btc_precision_fixed_{stamp}.png")
    ...
    if not richness["usable"]:
        ...
        payload = {
            "experiment": "exp14_multifile_btc_precision",
    ...
    notes = "# exp14 multifile btc precision\n\n" + "\n".join([
        f"- timestamp: {timestamp_utc}",
        f"- train files:",
        *[f"  - `{name}`" for name in train_file_names],
        f"- test file: `{test_file.name}`",
        f"- symbol: `BTC-USD`",
        f"- mutation: lower event-loss weight + lower pos_weight + higher trade threshold, with multi-file holdout and prediction cap",
    ...
    payload = {
        ...
        "richness_gate": richness,
        "mutation": {
            "type": "walkforward_multifile_holdout_plus_precision_tuning",
    ...
    row_id = register_experiment_run(
        experiment="exp14_multifile_btc_precision",
        variant="tinytcn_h64_btc_walkforward_precision",
        timestamp_utc=timestamp_utc,
        git_commit=git_commit,
        symbol="BTC-USD",
        cadence="100ms",
