...
from deep_orderbook.replayer import EndReplay, ReplayerProtocol
...
class TradeEvent(BaseModel):
    ...
    def product_id(self) -> str:
        ...
        assert len(set([trade.product_id for trade in self.trades])) == 1, (
...
class CoinbaseMessage(md.Message):
    ...
    def symbol(self) -> str:
        ...
        assert len(set([event.product_id for event in self.events])) == 1, (
...
class CoinbaseFeed(BaseFeed):
    ...
    def __init__(
        ...
        replayer: ReplayerProtocol | None = None,
    ) -> None:
        settings = Settings()
        self.config = config
        ...
        self.depth_managers: dict[str, md.DepthCachePlus] = {s: md.DepthCachePlus() for s in self.markets}
        ...
        self.queue_one_sec: asyncio.Queue[md.MulitSymbolOneSecondEnds] = asyncio.Queue(16)
        self.is_live = False
    ...
    async def polarize(
        ...
        *,
        ...
        df = df.with_columns([
            pl.col("channel").cast(pl.Enum(["l2_data", "market_trades", "subscriptions"])),
    ...
    def cut_trade_tape(self) -> dict[str, list[md.Trade]]:
        ...
        return {symbol: self.depth_managers[symbol].trade_bunch.trades for symbol in self.markets}
        ...
        match message.channel:
            ...
            case "market_trades":
                ...
                assert isinstance(message.events[0], TradeEvent) and (
                    len(set([trade.product_id for trade in message.events[0].trades])) == 1
                )
                for event in message.events:
                    assert isinstance(event, TradeEvent)
                    if self.PRINT_MESSAGE:
                        logger.debug(f"{message.channel} {event.type} {message.timestamp}: {len(event.trades)}")
                    if self.PRINT_EVENTS:
                        logger.debug(event)
                    match event.type:
                        case "snapshot":
...
async def main() -> None:  # pragma: no cover
    ...
    async with CoinbaseFeed(markets=["ETH-USD", "BTC-USD"], feed_msg_queue=True) as feed:
