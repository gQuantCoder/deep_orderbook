...
class BookShapper:
    ...
    async def on_depth_msg_async(self, msg: md.BookUpdate):
    ...
    async def on_trades_bunch(self, list_trades, force_t_avail=None):
        list_trades = [
            {k: float(v) for k, v in trs if k not in "Msea"}
    ...
    def trades2frame(list_trades):
        ...
        ts = ts.drop(["E", "T", "first_trade_id", "last_trade_id", "is_buyer_maker"], axis=1)
    ...
    @staticmethod
    def build(
        total, element, reduce_func=None, max_length=None, side_bips=None, side_width=None
    ):
